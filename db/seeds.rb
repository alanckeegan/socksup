...
Listing.create!(title: 'Bar Maid',hours_per_week: 25, pay_per_hour: 11, start_date: '15-06-2019'.to_datetime, end_date: '15-08-2019'.to_datetime, description: 'We are looking for reliable, personable, well presented waiting bartenders for our amazing club which prides itsef in telling local coding bootcamp students they are not dressed well enough to enter. Casual flexible position.',  employer_id: sutton.id, quiz_id: suttonquiz.id, photo: 'https://images.unsplash.com/photo-1516450360452-9312f5e86fc7?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=1650&q=80')

puts 'Creating 2 submissions...'

Submission.create!(user_id: marina.id, listing_id: suttonlisting.id, submitted: true)
Submission.create!(user_id: marina.id, listing_id: sunsetgrilllisting.id, submitted: true)

puts 'Creating 1 confirmed submission'
Submission.create!(user_id: marina.id, listing_id: nvclisting.id, submitted: true, confirmed: true)
